...
declare variable $poly:solids := doc("/db/apps/3d/data/solids.xml")/solids;
...
declare variable $poly:groups := doc("/db/apps/3d/data/groups.xml")/groups;
...
declare function poly:make-openscad($solid) {
       ...
       let $coordinates-os := if ($source="coordinates")
                              then concat ("function Z() = ",$cleanid,"();&#10;",poly:solid-to-openscad($solid,$cleanid)  )
                              else ()
